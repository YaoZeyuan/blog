  ...
  - [为什么 2021 年浏览器依然不能很好地复制动图?--紫云飞的回答](https://www.zhihu.com/question/460459199/answer/1899218661)
    ...
    - ```html
      ...
      <script type="module">
        ...
        async function showClipboardItems() {
          ...
          const clipboardItems = await navigator.clipboard.read({ raw });
          for (const item of clipboardItems) {
            for (const type of item.types) {
              await showClipboardData(await item.getType(type));
            }
          }
        }

        async function showClipboardData(blob) {
          console.log(blob);
          const fieldset = $("fieldset");
          $("legend", fieldset).textContent = blob.type;
          if (/image|png/i.test(blob.type)) {
            $("img", fieldset).src = URL.createObjectURL(blob);
          } else if (/tiff/i.test(blob.type)) {
            try {
          ...
          } else if (blob.type.startsWith("text") || blob.size < 100000) {
            $("textarea", fieldset).value = await blob.text();
          } else {
            $("span", fieldset).textContent =
              "blob 数据太大，无法显示，查看控制台";
            const link = $("a", fieldset);
            link.textContent = "或下载该文件";
            link.href = URL.createObjectURL(blob);
            link.download = blob.type;
          }
        }

        function $(tag, parent = document.body) {
